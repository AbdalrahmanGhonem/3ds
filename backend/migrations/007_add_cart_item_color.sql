...
SET @has_cart_product := (
  SELECT COUNT(*)
  FROM information_schema.statistics
  WHERE table_schema = DATABASE()
    AND table_name = 'cart_items'
    AND index_name = 'cart_product'
);
SET @sql := IF(
  @has_cart_product > 0,
  'ALTER TABLE cart_items DROP INDEX cart_product',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @has_cart_product_color := (
  SELECT COUNT(*)
  FROM information_schema.statistics
  WHERE table_schema = DATABASE()
    AND table_name = 'cart_items'
    AND index_name = 'cart_product_color'
);
SET @sql := IF(
  @has_cart_product_color = 0,
  'ALTER TABLE cart_items ADD UNIQUE KEY cart_product_color (cart_id, product_id, selected_color)',
  'SELECT 1'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
